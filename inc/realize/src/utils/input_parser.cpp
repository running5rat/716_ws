...
inline TimeWindow get_time_window(const rapidjson::Value& tw) {
  if (!tw.IsArray() or tw.Size() < 2 or !tw[0].IsUint() or !tw[1].IsUint()) {
    if (!tw.IsArray()) throw Exception(ERROR::INPUT, "1: Invalid time-window.");
    if (tw.Size() < 2) throw Exception(ERROR::INPUT, "2: Invalid time-window.");
    if (!tw[0].IsUint()) throw Exception(ERROR::INPUT, "3: Invalid time-window.");
    if (!tw[1].IsUint()) throw Exception(ERROR::INPUT, "4: Invalid time-window.");
